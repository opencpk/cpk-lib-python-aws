...
class AWSSSOAuditor:
    ...
    def get_permission_sets_for_account(self, account_id: str) -> List[str]:
        ...
        try:
            ...
            logger.info("Found %d permission sets provisioned to account %s", len(permission_sets), account_id)
            return permission_sets
        except Exception as e:
            logger.error("Error getting permission sets for account %s: %s", account_id, e)
            return []

    def get_account_assignments_for_permission_set(self, permission_set_arn: str, account_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error("Error getting account assignments for permission set %s: %s", permission_set_arn, e)
            return []

    def get_all_account_assignments(self, account_id: str) -> List[Dict[str, Any]]:
        """Get all account assignments for the given account by checking only provisioned permission sets."""
        all_assignments = []
        ...
        for permission_set_arn in permission_sets:
            assignments = self.get_account_assignments_for_permission_set(permission_set_arn, account_id)
    ...
    def get_permission_set_policies(self, permission_set_arn: str) -> Dict[str, Any]:
        ...
        try:
            ...
            managed_paginator = self.sso_admin_client.get_paginator('list_managed_policies_in_permission_set')
    ...
    def audit_account(self, account_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for assignment in assignments:
                ...
                if principal_type == 'GROUP':
                    if principal_id not in permission_sets_data[permission_set_arn]['AssignedGroups']:
                        permission_sets_data[permission_set_arn]['AssignedGroups'].append(principal_id)

            # Create simple lists for summary
            group_names = [group['DisplayName'] for group in groups_data.values()]
            permission_set_names = [ps.get('Name', 'Unknown') for ps in permission_sets_data.values()]
